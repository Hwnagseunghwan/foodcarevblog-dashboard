...
def load_all():
    ...
    # 1. 블로그 일별 조회수
    blog_daily = _get("blog_visitors.json", {})

    # 2. 블로그 월별 조회수
    blog_monthly = _get("blog_visitors_monthly.json", {})

    # 3. Blog Vola 일별 클릭수
    blog_vola_daily = {}
    vola_raw = _get("vola_clicks.json", {})
    for date_str, links in vola_raw.get("daily", {}).items():
        blog_vola_daily[date_str] = sum(v.get("daily_clicks", 0) for v in links.values())

    # 4. Blog 업무 데이터
    work_rows = _get("work_data.json", {}).get("rows", [])

    # 5. Seeding 업무 데이터
    seeding_rows = _get("seeding_data.json", {}).get("rows", [])

    # 6. Seeding Vola 일별 클릭수
    seeding_vola_rows = _get("seeding_vola_data.json", {}).get("rows", [])
